...
namespace DALAdministracion.LogicaNegocio
{
    ...
    /// </summary>
    public class LNCuentasBancarias_V2
    {
        ...
        public static DataSet ConsultaCuentas(DatosPersonalesCuenta losDatos, IUsuario usuario, Guid AppID)
        {
            ...
            try
            {
                return DAOCuentasBancarias_V2.ObtieneCuentas(losDatos, usuario, AppID);
            }
            ...
            try
            {
                ...
                using (SqlConnection conn = BDAutorizadorMC.BDEscritura)
                {
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOCuentasBancarias_V2.ActualizaDatosPersonalesCuenta(losDatos, usuario);
            ...
            try
            {
                using (SqlConnection conn = BDAutorizadorMC.BDEscritura)
                {
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOCuentasBancarias_V2.ActualizaParametroProductoCuenta(IdCuenta, IdProducto, NombreParametro, Valor, usuario);
                            transaccionSQL.Commit();
                        }
                        catch (CAppException err)
                        {
        ...
        /// <summary>
        /// Establece las condiciones de validación para la actualización de parámetros del 
        /// medio de acceso indicado en base de datos
        /// </summary>
        /// <param name="IdProducto">Identificador del medio de acceso por actualizar</param>
        /// <param name="NombreParametro">Nombre del parámetro por actualizar</param>
        /// <param name="Valor">Nuevo valor del parámetro por actualizar</param>
        /// <param name="usuario">Usuario en sesión</param>
        public static void ActualizaParametrosMA(Int64 IdProducto, String NombreParametro, String Valor, IUsuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDAutorizadorMC.BDEscritura)
                {
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOCuentasBancarias_V2.ActualizaParametrosMedioAcceso(IdProducto, NombreParametro, Valor, usuario);
                            transaccionSQL.Commit();
                        }
                        catch (CAppException err)
                        {
        ...
        /// <summary>
        /// Establece las condiciones de validación del acceso a datos para la solicitud
        /// de un cambio del Límte de Crédito
        /// </summary>
        /// <param name="IdCuenta">Identificador de la cuenta</param>
        /// <param name="IdProducto">Identificador del producto</param>
        ...
        /// <param name="usuario">Usuario en sesión</param>
        /// <param name="AppID">Identificador de la aplicación</param>
        public static void SolicitaCambioLimiteCredito(Int64 IdCuenta, Int64 IdProducto, string LDCNuevo, string PaginaAspx,
            ...
            try
            {
                using (SqlConnection conn = BDAutorizadorMC.BDEscritura)
                {
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOCuentasBancarias_V2.ControlaCambioLimiteCredito(IdCuenta, IdProducto, LDCNuevo, PaginaAspx, usuario, AppID);
                            transaccionSQL.Commit();
                        }
                        catch (CAppException err)
                        {
                        ...
                        {
                            transaccionSQL.Rollback();
                            throw new CAppException(8006, "Falla en el registro de cambio al límite de crédito de la cuenta en Base de Datos ", err);
                        }
                    }
                }
            }
            ...
            using (SqlConnection conn = BDAutorizadorMC.BDEscritura)
            {
                ...
                using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        DataSet dsDatosEvento = DAOCuentasBancarias_V2.ConsultaEventoAjusteManualLDC(IdColectiva, usuario, AppID);
        ...
        /// <param name="usuario">Usuario en sesión</param>
        public static void RegistraCambioLimiteCredito(Int64 IdCuenta, string NuevoLDC, IUsuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDAutorizadorMC.BDEscritura)
                {
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOCuentasBancarias_V2.RegistraCambioLimiteCredito(IdCuenta, NuevoLDC, usuario);
                            transaccionSQL.Commit();
                        }
                        catch (CAppException err)
                        {
                        ...
                        {
                            transaccionSQL.Rollback();
                            throw new CAppException(8006, "Falla al registrar el cambio en el límite de crédito en Base de Datos ", err);
                        }
                    }
                }
            }

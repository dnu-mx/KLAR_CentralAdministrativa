...
namespace DALCentroContacto.BaseDatos
{
    ...
    /// </summary>
    public class DAOConsultaClientesCash
    {
        /// <summary>
        ...
        public static DataSet ObtieneClientes(ClienteColectiva elCliente, IUsuario elUsuario, Guid AppID)
        {
            try
            {
        ...
        public static DataTable ObtieneDatosCliente(int idColectiva, IUsuario elUsuario)
        {
            try
            {
                SqlDatabase database = new SqlDatabase(BDAutorizadorCash.strBDLectura);
        ...
        public static void ActualizaDatosCliente(ClienteColectiva datosCliente, IUsuario elUsuario)
        {
            try
            {
                ...
                DbCommand command = database.GetStoredProcCommand("web_CA_ActualizaDatosColectiva");

                database.AddInParameter(command, "@IdColectiva", DbType.Int64, datosCliente.ID_Colectiva);
                ...
                database.AddInParameter(command, "@FechaNac", DbType.DateTime,
                    DateTime.Compare(datosCliente.FechaNacimiento, DateTime.MinValue) == 0 ? (DateTime?)null : datosCliente.FechaNacimiento);
                database.AddInParameter(command, "@CP", DbType.String, datosCliente.CodigoPostal);
                database.AddInParameter(command, "@RFC", DbType.String, datosCliente.RFC);
        ...
        public static DataSet ObtieneAliasDireccionesCliente(int idCliente, IUsuario elUsuario)
        {
            try
            {
                SqlDatabase database = new SqlDatabase(BDEcommerceCash.strBDLectura);
        ...
        public static DataSet ObtieneMovimientos(DateTime fIni, DateTime fFin, int idCol, IUsuario elUsuario, Guid AppID)
        {
            ...
            {
                SqlDatabase database = new SqlDatabase(BDAutorizadorCash.strBDLectura);
                DbCommand command = database.GetStoredProcCommand("web_CA_ObtieneMovimientosColectiva");
        ...
        /// <summary>
        /// Activa o inactiva el medio de acceso (teléfono) del cliente en base de datos
        /// </summary>
        /// <param name="telefono">Teléfono (medio de acceso) del cliente</param>
        /// <param name="idEstatus">Identificador del estatus de medio de acceso por establecer</param>
        /// <param name="elUsuario">Usuario en sesión</param>
        public static void ActivaInactivaMedioAcceso(String telefono, int idEstatus, IUsuario elUsuario)
        {
            try
            {
                SqlDatabase database = new SqlDatabase(BDAutorizadorCash.strBDEscritura);
                ...
                database.AddInParameter(command, "@ClaveMA", DbType.String, telefono);
                database.AddInParameter(command, "@IdEstatusMA", DbType.Int32, idEstatus);

                database.ExecuteNonQuery(command);

                Loguear.Evento("Se Actualizó el Estatus del Medio de Acceso " + telefono +
                    " en el Autorizador", elUsuario.ClaveUsuario);
            }
        ...
        /// <summary>
        /// Consulta en base de datos el saldo de la cuenta de la coletiva
        /// </summary>
        /// <param name="idColectiva">Identificador de la colectiva</param>
        /// <param name="elUsuario">Usuario en sesión</param>
        /// <returns>DataSet con los registros</returns>
        public static string ConsultaSaldoColectiva(int idColectiva, IUsuario elUsuario)
        {
            try
            {
                SqlDatabase database = new SqlDatabase(BDAutorizadorCash.strBDLectura);
                DbCommand command = database.GetStoredProcCommand("web_CA_ObtieneSaldoCuentaCliente");

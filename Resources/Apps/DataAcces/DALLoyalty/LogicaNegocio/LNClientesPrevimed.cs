...
{
    /// <summary>
    /// Establece la lógica del negocio del acceso a datos de Clientes Previmed
    /// </summary>
    public class LNClientesPrevimed
    {
        ...
        public static DataSet ConsultaClientes(ClienteColectiva cliente, Usuario usuario)
        {
            ...
            try
            {
                return DAOConsultaClientesPrevimed.ObtieneClientes(cliente, usuario);
            }
        ...
        public static void ActualizaEstatusConfirmaClientes(int IdEstatus, int IdColectiva, Usuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDAutorizadorPrevimed.BDEscritura)
                {
                    conn.Open();
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesPrevimed.ActualizaEstatusConfirma(IdColectiva, IdEstatus, usuario);
            ...
            try
            {
                using (SqlConnection conn = BDAutorizadorPrevimed.BDEscritura)
                {
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesPrevimed.ActualizaContrasenaCliente(Correo, Contrasena, usuario);
                            DAOConsultaClientesPrevimed.ActualizaEstatusConfirma(IdColectiva, 1, usuario);
        ...
        public static void AplicaBloqueoContracargoCliente(string Correo, bool Estatus, string Comentarios, Usuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDAdminPrevimed.BDEscritura)
                {
                    conn.Open();
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesPrevimed.ActualizaEstatusBloqueoCliente(Correo, Estatus, Comentarios, usuario);
        ...
        public static void AplicaCambioMembresia(string Correo, string Membresia, Usuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDEcommercePrevimed.BDEscritura)
                {
                    conn.Open();
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesPrevimed.ActualizaMembresiaCliente(Correo, Membresia, usuario);

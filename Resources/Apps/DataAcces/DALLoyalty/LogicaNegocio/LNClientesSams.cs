...
namespace DALCentroContacto.LogicaNegocio
{
    ...
    public class LNClientesSams
    {
        ...
        public static DataSet ConsultaClientes(ClienteColectiva cliente, Usuario usuario)
        {
            ...
            try
            {
                return DAOConsultaClientesSams.ObtieneClientes(cliente, usuario);
            }
        ...
        public static void ActualizaEstatusConfirmaClientes(int IdEstatus, int IdColectiva, Usuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDAutorizadorPromociones.BDEscritura)
                {
                    conn.Open();
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesSams.ActualizaEstatusConfirma(IdColectiva, IdEstatus, usuario);
            ...
            try
            {
                using (SqlConnection conn = BDAutorizadorPromociones.BDEscritura)
                {
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesSams.ActualizaContrasenaCliente(Correo, Contrasena, usuario);
                            DAOConsultaClientesSams.ActualizaEstatusConfirma(IdColectiva, 1, usuario);
        ...
        public static void AplicaBloqueoContracargoCliente(string Correo, bool Estatus, string Comentarios, Usuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDAdminPromociones.BDEscritura)
                {
                    conn.Open();
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesSams.ActualizaEstatusBloqueoCliente(Correo, Estatus, Comentarios, usuario);
            ...
            try
            {
                using (SqlConnection conn = BDEcommercePrana.BDEscritura)
                {
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesSams.ActualizaMembresiaCliente(Correo, Membresia, usuario);

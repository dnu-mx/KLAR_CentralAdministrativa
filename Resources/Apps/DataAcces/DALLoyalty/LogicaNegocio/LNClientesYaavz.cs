...
{
    /// <summary>
    /// Establece la lógica del negocio del acceso a datos de Clientes Yaavz
    /// </summary>
    public class LNClientesYaavz
    {
        ...
        public static DataSet ConsultaClientes(ClienteColectiva cliente, Usuario usuario)
        {
            ...
            try
            {
                return DAOConsultaClientesYaavz.ObtieneClientes(cliente, usuario);
            }
        ...
        public static void ActualizaEstatusConfirmaClientes(int IdEstatus, int IdColectiva, Usuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDAutorizadorYaavz.BDEscritura)
                {
                    conn.Open();
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesYaavz.ActualizaEstatusConfirma(IdColectiva, IdEstatus, usuario);
            ...
            try
            {
                using (SqlConnection conn = BDAutorizadorYaavz.BDEscritura)
                {
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesYaavz.ActualizaContrasenaCliente(Correo, Contrasena, usuario);
                            DAOConsultaClientesYaavz.ActualizaEstatusConfirma(IdColectiva, 1, usuario);
        ...
        public static void AplicaBloqueoContracargoCliente(string Correo, bool Estatus, string Comentarios, Usuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDAdminYaavz.BDEscritura)
                {
                    conn.Open();
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesYaavz.ActualizaEstatusBloqueoCliente(Correo, Estatus, Comentarios, usuario);
        ...
        public static void AplicaCambioMembresia(string Correo, string Membresia, Usuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDEcommerceYaavz.BDEscritura)
                {
                    conn.Open();
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesYaavz.ActualizaMembresiaCliente(Correo, Membresia, usuario);

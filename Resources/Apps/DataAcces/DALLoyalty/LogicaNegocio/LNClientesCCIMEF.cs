...
namespace DALCentroContacto.LogicaNegocio
{
    ...
    public class LNClientesCCIMEF
    {
        ...
        public static DataSet ConsultaClientes(ClienteColectiva cliente, Usuario usuario)
        {
            ...
            try
            {
                return DAOConsultaClientesCCIMEF.ObtieneClientes(cliente, usuario);
            }
        ...
        public static void ActualizaEstatusConfirmaClientes(int IdEstatus, int IdColectiva, Usuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDAutorizadorCCIMEF.BDEscritura)
                {
                    conn.Open();
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesCCIMEF.ActualizaEstatusConfirma(IdColectiva, IdEstatus, usuario);
            ...
            try
            {
                using (SqlConnection conn = BDAutorizadorCCIMEF.BDEscritura)
                {
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesCCIMEF.ActualizaContrasenaCliente(Correo, Contrasena, usuario);
                            DAOConsultaClientesCCIMEF.ActualizaEstatusConfirma(IdColectiva, 1, usuario);
        ...
        public static void AplicaBloqueoContracargoCliente(string Correo, bool Estatus, string Comentarios, Usuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDAdminCCIMEF.BDEscritura)
                {
                    conn.Open();
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesCCIMEF.ActualizaEstatusBloqueoCliente(Correo, Estatus, Comentarios, usuario);
        ...
        public static void AplicaCambioMembresia(string Correo, string Membresia, Usuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDEcommerceCCIMEF.BDEscritura)
                {
                    conn.Open();
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesCCIMEF.ActualizaMembresiaCliente(Correo, Membresia, usuario);

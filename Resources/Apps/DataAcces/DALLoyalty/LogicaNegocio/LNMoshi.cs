...
namespace DALCentroContacto.LogicaNegocio
{
    ...
    public class LNMoshi
    {
        ...
        public static DataSet ConsultaClientes(ClienteColectiva cliente, Usuario usuario)
        {
            ...
            try
            {
                return DAOConsultaClientesMoshi.ObtieneClientes(cliente, usuario);
            }
        ...
        public static void ActualizaDatosCliente(ClienteColectiva elCliente, Usuario usuario)
        {
            try
            {
                ...
                if (String.IsNullOrEmpty(elCliente.Nombre) && String.IsNullOrEmpty(elCliente.ApellidoPaterno) &&
                    String.IsNullOrEmpty(elCliente.ApellidoMaterno) && DateTime.Compare(elCliente.FechaNacimiento, DateTime.MinValue) == 0)
                {
                ...
                using (SqlConnection conn = BDEcommerce.BDEscritura)
                {
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesMoshi.ActualizaDatosCliente(elCliente, usuario);
        ...
        /// <summary>
        /// Establece y configura los parámetros para la ejecución del registro del evento manual
        /// "Ajuste Cargo Visitas"
        /// </summary>
        /// <param name="evento">Datos del evento</param>
        /// <param name="usuario">Usuario en sesión</param>
        public static void RegistraEvManual_AjusteCargoVisitas(EventosManuales evento, Usuario usuario)
        {
            using (SqlConnection conn = BDAutorizador.BDEscritura)
            {
                conn.Open();
                ...
                using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        losParametros["@ID_CuentaHabiente"] = new Parametro()
                        {
                         ...
                         losParametros["@Importe"] = new Parametro() { Nombre = "@Importe", 
                            Valor = evento.Importe, Descripcion = "Importe" };
                        losParametros["@Visitas"] = new Parametro() { Nombre = "@Visitas",
                            Valor = evento.Importe, Descripcion = "Visitas" };
        ...
        /// <summary>
        /// Establece y configura los parámetros para la ejecución del registro del evento manual
        /// "Ajuste Abono Visitas"
        /// </summary>
        /// <param name="evento">Datos del evento</param>
        /// <param name="usuario">Usuario en sesión</param>
        public static void RegistraEvManual_AjusteAbonoVisitas(EventosManuales evento, Usuario usuario)
        {
            using (SqlConnection conn = BDAutorizador.BDEscritura)
            {
                conn.Open();

...
namespace DALCentroContacto.LogicaNegocio
{
    ...
    /// </summary>
    public class LNClientesCuponClick
    {
        ...
        public static DataSet ConsultaClientes(ClienteColectiva cliente, Usuario usuario)
        {
            ...
            try
            {
                return DAOConsultaClientesCuponClick.ObtieneClientes(cliente, usuario);
            }
        ...
        public static void ActualizaEstatusConfirmaClientes(int IdEstatus, int IdColectiva, Usuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDAutorizadorCuponClick.BDEscritura)
                {
                    conn.Open();
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesCuponClick.ActualizaEstatusConfirma(IdColectiva, IdEstatus, usuario);
            ...
            try
            {
                using (SqlConnection conn = BDAutorizadorCuponClick.BDEscritura)
                {
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesCuponClick.ActualizaContrasenaCliente(Correo, Contrasena, usuario);
                            DAOConsultaClientesCuponClick.ActualizaEstatusConfirma(IdColectiva, 1, usuario);
        ...
        public static void AplicaBloqueoContracargoCliente(string Correo, bool Estatus, string Comentarios, Usuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDAdminCuponClick.BDEscritura)
                {
                    conn.Open();
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesCuponClick.ActualizaEstatusBloqueoCliente(Correo, Estatus, Comentarios, usuario);
        ...
        public static void AplicaCambioMembresia(string Correo, string Membresia, Usuario usuario)
        {
            try
            {
                using (SqlConnection conn = BDEcommerceCuponClick.BDEscritura)
                {
                    conn.Open();
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            DAOConsultaClientesCuponClick.ActualizaMembresiaCliente(Correo, Membresia, usuario);

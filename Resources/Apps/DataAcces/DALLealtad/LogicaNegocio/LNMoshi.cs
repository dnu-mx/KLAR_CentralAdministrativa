...
{
    /// <summary>
    /// Establece la lógica del negocio del nivel de acceso a datos de Moshi
    /// </summary>
    public class LNMoshi
    {
                ...
                using (SqlConnection conn = BDAutorizador.BDEscritura)
                {
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            ...
                            {
                                DAOMoshi.ActualizaValorReglaPorGpoCuenta(unValor, Id_cadena, ID_Regla, IdGpoCuenta, elUser);
                            }
        ...
        /// <summary>
        /// Establece las condiciones de validación para la modificación de los valores de los parámetros
        /// de la regla indicada, para el grupo de medios de acceso indicado
        /// </summary>
        /// <param name="losValoresRegla">Lista con los valores de la regla por modificar</param>
        /// <param name="Id_cadena">Identificador de la cadena comercial</param>
        /// <param name="ID_Regla">Identificador de la regla</param>
        ...
        /// <param name="elUser">Usuario en sesión</param>
        public static void ModificaValoresReglaPorGrupoMA(List<ValorRegla> losValoresRegla, Int64 Id_cadena, Int64 ID_Regla,
            int IdGrupoMA, IUsuario elUser)
        {
            try
            {
                using (SqlConnection conn = BDAutorizador.BDEscritura)
                {
                    ...
                    using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                    {
                        try
                        {
                            foreach (ValorRegla unValor in losValoresRegla)
                            {
                                DAOMoshi.ActualizaValorReglaPorGrupoMA(unValor, Id_cadena, ID_Regla, IdGrupoMA, elUser);
                            }

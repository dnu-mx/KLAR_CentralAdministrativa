...
{
    public partial class SaldosCadenaComercial : DALCentralAplicaciones.PaginaBaseCAPP
    {
        #region Variables privadas

        //LOG HEADER Saldos Cadena Comercial
        private LogHeader LH_SaldosCadComercial = new LogHeader();
        ...
        {
            ///LOG HEADER
            LH_SaldosCadComercial.IP_Address = Request.ServerVariables["REMOTE_ADDR"].ToString();
            LH_SaldosCadComercial.Application_ID = Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString());
            LH_SaldosCadComercial.User = this.Usuario.ClaveUsuario;
            LH_SaldosCadComercial.Trace_ID = Guid.NewGuid();

            LogPCI log = new LogPCI(LH_SaldosCadComercial);

            try
            {
                log.Info("INICIA SaldosCadenaComercial Page_Load()");
                ...
                log.Info("TERMINA SaldosCadenaComercial Page_Load()");
            }
            catch (Exception err)
            {
        ...
        public void AfterEdit(int ID_Cuenta, string newValue, Int64 ID_Colectiva, string TipoCuentaISO, string codigoMoneda, bool AceptaEditar , string afiliacion)
        {
            ...
            LogPCI unLog = new LogPCI(LH_SaldosCadComercial);

            try
            {
                unLog.Info("INICIA ModificarSaldo()");
                resp = LNSaldos.ModificarSaldo(float.Parse(newValue), ID_Cuenta, ID_Colectiva, TipoCuentaISO, codigoMoneda,
                    AceptaEditar, AppId, this.Usuario, afiliacion, LH_SaldosCadComercial);
                unLog.Info("INICIA ModificarSaldo()");
                ...
                else
                {
                    if (resp.IsTimeOut)
                    {
                        ...
                        X.Msg.Notify("Respuesta Transaccional", "Modificación de Saldo <br /><br />  <b> T I M E    O U T </b> <br />  <br /> ").Show();
                    }
                    else
                    {
                        ...
                        {
                            X.Msg.Notify("Respuesta Transaccional", "Mensaje: <b>" + resp.ResultadoOperacion + "</b> <br />").Show();
                            X.Msg.Notify("Respuesta Transaccional", "Modificación de Saldo <br /><br />  <b> D E C L I N A D A </b> <br />  <br /> ").Show();
                        }
                        else
                        {
                            X.Msg.Notify("Respuesta Transaccional", "<br />  Código Respuesta: <b>" + resp.CodigoRespuesta + "</b> <br /> Numero Autorización: <b>" + resp.Autorizacion + "</b> <br />" + "Mensaje: <b>" + resp.DescripcionRespuesta + "</b> <br />").Show();
                            X.Msg.Notify("Respuesta Transaccional", "Modificación de Saldo <br /><br />  <b> D E C L I N A D A </b> <br />  <br /> ").Show();
                        }
                    }
                }
        ...
        {
            LogPCI pCI = new LogPCI(LH_SaldosCadComercial);
            ...
            try
            {
                ...
                GridPanel2.DirectEvents.RowDblClick.ExtraParams.Add(new Parameter("Afiliación", "this.getRowsValues({ selectedOnly: true })[0].AfiliacionAdmin", ParameterMode.Raw));
        ...
        protected void btnCrearCta_Click(object sender, DirectEventArgs e)
        {
            ...
            try
            {
                ...
                log.Info("INICIA AgregarCuentasAColectiva()");
                bool agrega = LNCuenta.AgregarCuentasAColectiva(ID_colectiva, this.Usuario,
                    Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()),
                    LH_SaldosCadComercial);
                log.Info("TERMINA AgregarCuentasAColectiva()");
        ...
        {
            LogPCI logPCI = new LogPCI(LH_SaldosCadComercial);
            logPCI.Info("PreparaGridSaldos()");
        ...
        private void BindDataEmpleados()
        {
            ...
            GridPanel2.GetStore().DataSource = DAOCatalogos.ListaEmpleadosMiembrosClubGrupoComercial(-1, 
                Configuracion.Get(IDApp, "ClaveTipoColectivaCadenaComercial").Valor, this.Usuario,
                Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString()),
                LH_SaldosCadComercial);
        ...
        protected void GridEmpleados_DblClik(object sender, DirectEventArgs e)
        {
            ...
            try
            {
                ...
                Panel5.Title = "Saldos en Cuentas de la Cadena Comercial No. [" + unvalor + "]";

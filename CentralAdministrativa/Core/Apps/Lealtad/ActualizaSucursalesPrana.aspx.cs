...
namespace Lealtad
{
    public partial class ActualizaSucursalesPrana : DALCentralAplicaciones.PaginaBaseCAPP
    {
        /// <summary>
        ...
        protected void ValidaMuestraYSubeArchivo(DataSet ds)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                {
                    X.Msg.Alert("Actualizar Sucursales",
                        "El Pais no puede estar vacío. <b>(Fila no. " +
                        (rowIndex + 2).ToString() + ")</b>").Show();
                    return;
                }
                ...
                {
                    if (!Decimal.TryParse(row.ItemArray[10].ToString(), out latitud))
                    {
                ...
                {
                    if (!Decimal.TryParse(row.ItemArray[11].ToString(), out longitud))
                    {
                ...
                {
                    dt.Rows[rowIndex][12] = row.ItemArray[12].ToString().Trim().ToUpper() == "SI" ? 1 : 0;
                }

                if (String.IsNullOrEmpty(row.ItemArray[13].ToString()))
                {
                    X.Msg.Alert("Actualizar Sucursales",
                        "La Clasificación no puede estar vacía. <b>(Fila no. " +
                        (rowIndex + 2).ToString() + ")</b>").Show();
                    return;
                }
            ...
            dt.AcceptChanges();
            LNEcommercePrana.InsertaSucursalesTMP(dt, this.Usuario);
        ...
        protected void NombreColumnas(DataTable dt_)
        {
            ...
            dt_.Columns[7].ColumnName = "Pais";
            dt_.Columns[8].ColumnName = "Estado";
            dt_.Columns[9].ColumnName = "Telefono";
            dt_.Columns[10].ColumnName = "Latitud";
            dt_.Columns[11].ColumnName = "Longitud";
            dt_.Columns[12].ColumnName = "Activa";
        }
        ...
        public void btnAplicarCambios_Click(object sender, DirectEventArgs e)
        {
            try
            {
                string respuestaBD = LNEcommercePrana.AplicaCambiosASucursales(this.Usuario);

...
namespace Lealtad
{
    public partial class ReporteEnvioCorreosVentaSAMS : DALCentralAplicaciones.PaginaBaseCAPP
    {
        /// <summary>
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    HttpContext.Current.Session.Add("DtCorreosVentas", null);
                }
        ...
        protected void LlenaGridPanelCorreos(int RegistroInicial, string Columna, SortDirection Orden)
        {
            this.btnExportExcel.Disabled = true;
            this.btnExportCSV.Disabled = true;
            ...
            try
            {
                DataTable dtCorreosVentas = new DataTable();

                dtCorreosVentas = HttpContext.Current.Session["DtCorreosVentas"] as DataTable;

                if (dtCorreosVentas == null)
                {
                    dtCorreosVentas = DAOEcommerceRedVoucher.ObtieneReporteEnvioCorreoVentasSams(
                    ...
                    HttpContext.Current.Session.Add("DtCorreosVentas", dtCorreosVentas);
                }
                ...
                if (dtCorreosVentas.Rows.Count < 1)
                {
                    X.Msg.Alert("Correos Ventas Sams", "No existen coincidencias con la búsqueda solicitada").Show();
                }
                else if (dtCorreosVentas.Rows.Count > maxRegistros)
                {
                    X.Msg.Confirm("Correos Ventas Sams", "La búsqueda arrojó más de " + maxRegistros.ToString() + " registros.</br>" +
                ...
                {
                    int TotalRegistros = dtCorreosVentas.Rows.Count;
                    
                    (this.StoreCorreos.Proxy[0] as PageProxy).Total = TotalRegistros;

                    DataTable sortedDT = dtCorreosVentas.Clone();
                    DataTable dtToGrid = dtCorreosVentas.Clone();

                    //Se ordenan los datos según la elección del usuario
                    if (!String.IsNullOrEmpty(Columna))
                    {
                        System.Data.DataView dv = dtCorreosVentas.DefaultView;
                    ...
                    {
                        dtToGrid.ImportRow(String.IsNullOrEmpty(Columna) ? dtCorreosVentas.Rows[row] : sortedDT.Rows[row]);
                    }

                    dtToGrid.AcceptChanges();

                    StoreCorreos.DataSource = dtToGrid;
                    StoreCorreos.DataBind();
            ...
            {
                X.Msg.Alert("Correos Ventas Sams", err.Mensaje()).Show();
            }

            catch (Exception ex)
            {
                DALLealtad.Utilidades.Loguear.Error(ex, this.Usuario.ClaveUsuario);
                X.Msg.Alert("Correos Ventas Sams", "Ocurrió un Error al Consultar los Correos de Ventas Sams").Show();
            }
        ...
        protected void StoreCorreos_RefreshData(object sender, StoreRefreshDataEventArgs e)
        {
            int inicio = e.Start;
            string columna = e.Sort;
            SortDirection orden = e.Dir;

            LlenaGridPanelCorreos(inicio, columna, orden);
        }
        ...
        public void ExportDataTableToExcel()
        {
            try
            {
                string reportName = "ReporteCorreosVentasSAMS";
                DataTable _dtCorreosVentas = HttpContext.Current.Session["DtCorreosVentas"] as DataTable;

                XLWorkbook wb = new XLWorkbook();
                var ws = wb.Worksheets.Add(_dtCorreosVentas, reportName);
            ...
            {
                DALLealtad.Utilidades.Loguear.Error(ex, this.Usuario.ClaveUsuario);
                X.Msg.Alert("Correos Ventas Sams", "Ocurrió un Error al Exportar el Reporte a Excel").Show();
            }
        }

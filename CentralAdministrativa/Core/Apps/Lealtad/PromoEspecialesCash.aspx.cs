...
namespace Lealtad
{
    public partial class PromoEspecialesCash : DALCentralAplicaciones.PaginaBaseCAPP
    {
        /// <summary>
        ...
        protected void LlenaComboNivelesLealtad()
        {
            try
            {
                ...
                this.StoreNivelLealtad.DataSource = DAOCash.ObtieneCatalogoNivelesLealtad(this.Usuario);
        ...
        protected void LlenaComboPromociones()
        {
            try
            {
                ...
                this.StorePromocion.DataSource = DAOCash.ObtieneCatalogoPromociones(this.Usuario);
        ...
        protected void select_Promocion(object sender, DirectEventArgs e)
        {
            try
            {
                DataTable dtParamsPromo = DAOCash.ObtieneParamsPromoEspecial(
        ...
        protected void LlenaGridPropiedades(Int64 ID_CadenaComercial, Int64 ID_Regla)
        {
            try
            {
                ...
                foreach (ValorRegla unaProp in DAOCash.ListaValoresReglaPromoEspecial(ID_CadenaComercial,
                    int.Parse(this.cBoxPromocion.SelectedItem.Value),
                    ...
                    this.Usuario, Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString())).Values)
                {
                    ...
                    else
                    {
                        if (unaProp.Nombre == Configuracion.Get(Guid.Parse(
                            ...
                            ||
                            unaProp.Nombre == Configuracion.Get(Guid.Parse(
                                ConfigurationManager.AppSettings["IDApplication"].ToString()), "ClaveValorReglaBeneficiarios").Valor)
                        {
                                ...
                                ConfigurationManager.AppSettings["IDApplication"].ToString()), "ClaveValorReglaDiasSemana").Valor)
                            {
                                ...
                                ConfigurationManager.AppSettings["IDApplication"].ToString()), "ClaveValorReglaSucursales").Valor)
                            {
                                DataSet dsSucursales = DAOCash.ObtieneCatalogoSucursales(this.Usuario,
                                ...
                                ConfigurationManager.AppSettings["IDApplication"].ToString()), "ClaveValorReglaBeneficiarios").Valor)
                            {
                                DataSet dsBeneficiarios = DAOCash.ObtieneCatalogoBeneficiarios(this.Usuario,
                                    Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString()));

                                for (int benef = 0; benef < dsBeneficiarios.Tables[0].Rows.Count; benef++)
                                {
                                    ListItem li = new ListItem();
                                    li.Value = dsBeneficiarios.Tables[0].Rows[benef]["ClaveColectiva"].ToString().Trim();
                                    li.Text = dsBeneficiarios.Tables[0].Rows[benef]["ClaveColectiva"].ToString().Trim();
        ...
        protected void btnGuardar_Click(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                //Guardar Valores
                LNCash.ModificaValoresReglaPorGpoCuenta(losCambios, Int64.Parse(hdnIdCadena.Value.ToString()),
                    Int64.Parse(hdnIdReglaPE.Value.ToString()),
                    Convert.ToInt32(this.cBoxNivelLealtad.SelectedItem.Value),
                    this.Usuario);

                LlenaGridPropiedades(Int64.Parse(hdnIdCadena.Value.ToString()),
                    Int64.Parse(hdnIdReglaPE.Value.ToString()));
        ...
        protected void btnAceptaNuevaPromo_Click(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                dsEvento = DAOCash.ObtieneDetallesEvento(this.Usuario);
                ...
                DAOCash.InsertaNuevaPromocionEspecial(IdEventoOrigen, ClaveNuevoEvento,

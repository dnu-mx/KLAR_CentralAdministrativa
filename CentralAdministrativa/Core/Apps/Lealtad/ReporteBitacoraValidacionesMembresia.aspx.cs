...
namespace Lealtad
{
    public partial class ReporteBitacoraValidacionesMembresia : PaginaBaseCAPP
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    HttpContext.Current.Session.Add("dtDatosReporte", null);
                }
            }
            catch (Exception err)
            {
        ...
        protected void LlenaGridPanel(int RegistroInicial, string Columna, SortDirection Orden)
        {
            ...
            try
            {
                DataTable dtDatosReporte = new DataTable();

                dtDatosReporte = HttpContext.Current.Session["DtDatosReporte"] as DataTable;

                if (dtDatosReporte == null)
                {
                    dtDatosReporte = DAOEcommercePrana.ObtieneReporteBitacoraMembresia(
                        Convert.ToDateTime(dfFechaInicio.SelectedDate),
                        Convert.ToDateTime(dfFechaFin.SelectedDate),
                        txtMembresia.Text,
                        this.Usuario);

                    HttpContext.Current.Session.Add("DtDatosReporte", dtDatosReporte);
                }
                ...
                if (dtDatosReporte == null)
                {
                    X.Msg.Alert("Reporte Bitacora Validaciones de Membresia", "No existen coincidencias con la búsqueda solicitada").Show();
                }
                else if (dtDatosReporte.Rows.Count > maxRegistros)
                {
                    X.Msg.Confirm("Reporte Bitacora Validaciones de Membresia", "La búsqueda arrojó más de " + maxRegistros.ToString() + " registros.</br>" +
                        "El reporte se exportará directamente al archivo Excel.", new MessageBoxButtonsConfig
                        {
                            Yes = new MessageBoxButtonConfig
                            {
                                Handler = "RepDatos.ClicDePaso()",
                                Text = "Aceptar"
                            },
                ...
                {
                    int TotalRegistros = dtDatosReporte.Rows.Count;

                    (this.StoreInfo.Proxy[0] as PageProxy).Total = TotalRegistros;

                    DataTable sortedDT = dtDatosReporte.Clone();
                    DataTable dtToGrid = dtDatosReporte.Clone();

                    //Se ordenan los datos según la elección del usuario
                    if (!String.IsNullOrEmpty(Columna))
                    {
                        System.Data.DataView dv = dtDatosReporte.DefaultView;
                    ...
                    {
                        dtToGrid.ImportRow(String.IsNullOrEmpty(Columna) ? dtDatosReporte.Rows[row] : sortedDT.Rows[row]);
                    }

                    dtToGrid.AcceptChanges();

                    StoreInfo.DataSource = dtToGrid;
                    StoreInfo.DataBind();
            ...
            {
                X.Msg.Alert("Reporte Bitacora Validaciones de Membresia", err.Mensaje()).Show();
            }

            catch (Exception ex)
            {
                Loguear.Error(ex, this.Usuario.ClaveUsuario);
                X.Msg.Alert("Reporte Bitacora Validaciones de Membresia", "Ocurrió un Error al Consultar el Reporte").Show();
            }
        ...
        protected void StoreInfo_RefreshData(object sender, StoreRefreshDataEventArgs e)
        {
            int inicio = e.Start;
            string columna = e.Sort;
            SortDirection orden = e.Dir;

            LlenaGridPanel(inicio, columna, orden);
        }
        ...
        /// </summary>
        [DirectMethod(Namespace = "RepDatos")]
        ...
        public void ExportDataTableToExcel()
        {
            try
            {
                string reportName = "ReporteBitacoraMembresias";
                DataTable _dtDatosReporte = HttpContext.Current.Session["DtDatosReporte"] as DataTable;

                XLWorkbook wb = new XLWorkbook();
                var ws = wb.Worksheets.Add(_dtDatosReporte, reportName);
            ...
            {
                Loguear.Error(ex, this.Usuario.ClaveUsuario);
                X.Msg.Alert("Reporte Bitacora Validaciones de Membresia", "Ocurrió un Error al Exportar el Reporte a Excel").Show();
            }
        }

...
namespace Lealtad
{
    public partial class CambioNivelMoshi : DALCentralAplicaciones.PaginaBaseCAPP
    {
        /// <summary>
        ...
        protected void select_NivelLealtad(object sender, DirectEventArgs e)
        {
            try
            {
                DataSet dsParamsRegla = DAOMoshi.ObtieneParamsRegla(Configuracion.Get(Guid.Parse(
                    ConfigurationManager.AppSettings["IDApplication"].ToString()), "ClaveReglaCambioNivel").Valor,
                ...
                HttpContext.Current.Session.Add("ID_ReglaCambio", IdRegla);
                HttpContext.Current.Session.Add("ID_CadenaMoshi", IdColectiva);

                LlenaGridPropiedades(IdColectiva, IdRegla);
            }

            catch (Exception ex)
            {
                X.Msg.Alert("Cambio de Nivel", "Ocurrió un error al consultar  los valores de la regla.").Show();
                DALLealtad.Utilidades.Loguear.Error(ex, this.Usuario.ClaveUsuario);
            }
        }
        ...
        /// <param name="ID_ReglaCambio">Identificador de la regla</param>
        protected void LlenaGridPropiedades(Int64 ID_CadenaComercial, Int64 ID_Regla)
        {
            try
            {
                ...
                foreach(ValorRegla unaProp in DAOMoshi.ListaValoresReglaPorGpoCta_SinValores(ID_Regla,
        ...
        protected void btnGuardar_Click(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                {
                    X.Msg.Notify("Cambio de Nivel", "Modificación de Valores de la Regla <br /><br />  <b> La Fecha Inicial debe ser menor o igual que la Fecha Final </b> <br />  <br /> ").Show();
                    return;
                }

                if (DateTime.Compare(laFechaFinal, DateTime.Now) == 0)
                {
                    X.Msg.Notify("Cambio de Nivel", "Modificación de Valores de la Regla <br /><br />  <b> La Fecha Final no puede ser igual a la actual </b> <br />  <br /> ").Show();
                    return;
                }

                //Guardar Valores

                LNRegla.ModificaValoresRegla(losCambios, Int64.Parse(HttpContext.Current.Session["ID_CadenaMoshi"].ToString()),
                    Int64.Parse(HttpContext.Current.Session["ID_ReglaCambio"].ToString()), this.Usuario);

                LlenaGridPropiedades(Int64.Parse(HttpContext.Current.Session["ID_CadenaMoshi"].ToString()), 
                    Int64.Parse(HttpContext.Current.Session["ID_ReglaCambio"].ToString()));

                X.Msg.Notify("Cambio de Nivel", "Modificación de Valores de la Regla <br /><br />  <b> E X I T O S O </b> <br />  <br /> ").Show();
            }

            catch (Exception err)
            {
                DALLealtad.Utilidades.Loguear.Error(err, this.Usuario.ClaveUsuario);
                X.Msg.Notify("Cambio de Nivel", "Modificación de Valores de la Regla <br /><br />  <b> D E C L I N A D O </b> <br />  <br /> ").Show();
            }
        }

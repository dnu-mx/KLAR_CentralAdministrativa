...
namespace Lealtad
{
    public partial class ReporteBeneficiosEdenred : DALCentralAplicaciones.PaginaBaseCAPP
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    DataSet dsCadenas = DAOEcommerceEdenred.ListaCadenas(this.Usuario);
                    ...
                    StoreGiros.DataSource = DAOEcommerceEdenred.ListaGiros(this.Usuario);
                    StoreGiros.DataBind();

                    StorePromociones.DataSource = DAOEcommerceEdenred.ListaPromociones(this.Usuario);
        ...
        protected void LlenaGridPanelBeneficios(int RegistroInicial, string Columna, SortDirection Orden)
        {
            ...
            try
            {
                ...
                if (dtBeneficios == null)
                {
                    ...
                    dtBeneficios = DAOEcommerceEdenred.ObtieneReportePromociones(
                ...
                else if (dtBeneficios.Rows.Count > maxRegistros)
                {
                    X.Msg.Confirm("Beneficios", "La búsqueda arrojó más de " + maxRegistros.ToString() + " registros.</br>" +
                        "El reporte se exportará directamente al archivo Excel.", new MessageBoxButtonsConfig
                        {
                            ...
                            {
                                Handler = "RepBenefEdn.ClicDePaso()",
                                Text = "Aceptar"
                            },
        ...
        /// </summary>
        [DirectMethod(Namespace = "RepBenefEdn")]

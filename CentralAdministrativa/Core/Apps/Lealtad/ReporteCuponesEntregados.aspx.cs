...
namespace Lealtad
{
    public partial class ReporteCuponesEntregados : DALCentralAplicaciones.PaginaBaseCAPP
    {
        /// <summary>
        ...
        /// <summary>
        /// Carga el catálogo de promociones de la cadena elegida, en el control destinado a ello,
        ...
        protected void LlenaPromociones(object sender, EventArgs e)
        {
            try
            {
                ...
                StorePromocion.DataSource = DAOPromociones.ListaPromocionesConCupones(
                    Convert.ToInt32(cBoxCadena.SelectedItem.Value), this.Usuario);
                StorePromocion.DataBind();
            }
            catch (CAppException caEx)
            {
                X.Msg.Alert("Cupones", caEx.Mensaje()).Show();
            }
            catch (Exception ex)
            {
                X.Msg.Alert("Cupones", ex.Message).Show();
            }
        }
        ...
        protected void LlenaGridPanelCupones(int RegistroInicial, string Columna, SortDirection Orden)
        {
            ...
            try
            {
                ...
                if (dtCupones == null)
                {
                    dtCupones = DAOPromociones.ObtieneReporteCuponesEntregados(
                        ...
                        String.IsNullOrEmpty(cBoxCadena.SelectedItem.Value) ? -1 :
                            Convert.ToInt32(cBoxCadena.SelectedItem.Value),
                ...
                {
                    X.Msg.Alert("Cupones", "No existen coincidencias con la búsqueda solicitada").Show();
                }
                else if (dtCupones.Rows.Count > maxRegistros)
                {
                    X.Msg.Confirm("Cupones", "La búsqueda arrojó más de " + maxRegistros.ToString() + " registros.</br>" +
            ...
            {
                X.Msg.Alert("Cupones", err.Mensaje()).Show();
            }

            catch (Exception ex)
            {
                DALLealtad.Utilidades.Loguear.Error(ex, this.Usuario.ClaveUsuario);
                X.Msg.Alert("Cupones", "Ocurrió un Error al Consultar los Cupones Entregados").Show();
            }
        ...
        public void ExportDataTableToExcel()
        {
            try
            {
                string reportName = "ReporteCuponesEntregados";
            ...
            {
                DALLealtad.Utilidades.Loguear.Error(ex, this.Usuario.ClaveUsuario);
                X.Msg.Alert("Cupones", "Ocurrió un Error al Exportar el Reporte a Excel").Show();
            }
        }

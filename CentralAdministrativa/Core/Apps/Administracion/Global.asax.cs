...
namespace Administracion
{
    public class Global : System.Web.HttpApplication
    {
        ...
        {
            ///LOG HEADER
            LogHeader LH_GlobalAdmin = new LogHeader();
            var ts = DateTime.Now.ToString("yyyyMMddHHmmss");
            GlobalContext.Properties["ts"] = ts;

            XmlConfigurator.Configure();

            LH_GlobalAdmin.Application_ID = Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString());
            LH_GlobalAdmin.Trace_ID = Guid.NewGuid();

            LogPCI log = new LogPCI(LH_GlobalAdmin);
            log.Info("INICIA Administracion_Application_Start()");
            ...
            try
            {
                ...
                ValoresInicial.InicializarContexto(LH_GlobalAdmin);

                log.Info("TERMINA Administracion_Application_Start()");
            }
            catch (Exception ex)
            {

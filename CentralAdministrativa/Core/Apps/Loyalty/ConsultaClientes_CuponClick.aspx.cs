...
{
    public partial class ConsultaClientes_CuponClick : DALCentralAplicaciones.PaginaBaseCAPP
    {
        ...
        protected void LlenarGridResultados()
        {
            try
            {
                ...
                elCliente.Membresia = this.txtMembresiaSearch.Text;

                DataSet dsResultados = LNClientesCuponClick.ConsultaClientes(elCliente, this.Usuario);
            ...
            try
            {
                ...
                switch (Formulario)
                {
                    case (ushort)AccionBitacora.CambioContrasena:
                        Accion = "Ajuste Manual Clientes CuponClick - Cambio de Contraseña";
                        break;

                    case (ushort)AccionBitacora.CambioEstatus:
                        Accion = "Ajuste Manual Clientes CuponClick - Confirmación de Correo";
                        break;

                    case (ushort)AccionBitacora.Bloqueo:
                        Accion = "Ajuste Manual Clientes CuponClick - Bloqueo/Desbloqueo";
                        break;

                    case (ushort)AccionBitacora.CambioMembresia:
                        Accion = "Ajuste Manual Clientes CuponClick - Cambio de Membresia";
                        break;
        ...
        protected void LlenaFieldSetDatosCliente(int idcliente, int idcolectiva, int idtipocolectiva, String eMail)
        {
            ...
            {
                FormPanelDatos.Reset();
                DataSet dsDatos = DAOConsultaClientesCuponClick.ObtieneDatosCliente(idcliente, idcolectiva, this.Usuario);
                ...
                this.txtMembresia.Text = dsDatos.Tables[0].Rows[0]["Membresia"].ToString().Trim();

                this.txtMembresiaDatos.Text = dsDatos.Tables[0].Rows[0]["Membresia"].ToString().Trim();
                this.txtNivel.Text = dsDatos.Tables[0].Rows[0]["Nivel"].ToString().Trim();

                if (dsDatos.Tables[0].Rows[0]["FechaRenovacion"].ToString() == "")
                {
                    this.dfFechaRenovacion.Clear();
                }
                else
                {
                    this.dfFechaRenovacion.Value = String.Format("{0:dd/MM/yyyy}",
                    Convert.ToDateTime(dsDatos.Tables[0].Rows[0]["FechaRenovacion"].ToString()));
                }

                if (dsDatos.Tables[0].Rows[0]["FechaNacimiento"].ToString() == "")
                {
                ...
                if (dsDatos.Tables[0].Rows[0]["FechaAlta"].ToString() == "")
                {
                ...
                if (dsDatos.Tables[0].Rows[0]["FechaAltaLealtad"].ToString() == "")
                {
                ...
                dsDatos = DAOConsultaClientesCuponClick.ObtieneBloqueoCliente(eMail, this.Usuario);
                string bloqueo;
        ...
        protected void btnGuardarEstatus_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    LNClientesCuponClick.ActualizaEstatusConfirmaClientes(Convert.ToInt32(cboEstatusConfirma.Value), Convert.ToInt32(this.hdID_Colectiva.Text), this.Usuario);
        ...
        protected void btnGenerarContrasena_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.hdeMail.Text != "")
                {
                    ...
                    LNClientesCuponClick.GenerarContrasenaClientes(Convert.ToInt32(this.hdID_Colectiva.Text), this.hdeMail.Text, contrasena, this.Usuario);
        ...
        protected void btnAplicarBloqueo_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.hdeMail.Text != "")
                {
                    ...
                    LNClientesCuponClick.AplicaBloqueoContracargoCliente(this.hdeMail.Text, rdBloqueado.Checked, txtComentario.Text, this.Usuario);
        ...
        /// <param name="sender">Objeto que envía el control</param>
        /// <param name="e">Argumentos del evento que se ejecutó</param>
        protected void btnCambiarMembresia_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.hdeMail.Text != "" && this.txtMembresiaNew.Text != "")
                {
                    LNClientesCuponClick.AplicaCambioMembresia(this.hdeMail.Text, this.txtMembresiaNew.Text, this.Usuario);
                    X.Msg.Notify("", "Cambio de Membresia Aplicada <br />  <br /> <b> E X I T O S A M E N T E </b> <br />  <br /> ").Show();

                    this.txtMembresia.Text = this.txtMembresiaNew.Text;
                    this.txtMembresiaNew.Text = "";

                    RegistraEnBitacora(Convert.ToInt32(this.hdID_Colectiva.Text), (int)AccionBitacora.CambioMembresia);
                }
            }
            ...
            {
                DALLoyalty.Utilidades.Loguear.Error(caEx, this.Usuario.ClaveUsuario);
                X.Msg.Alert("Cambio de Membresia", caEx.Mensaje()).Show();
            }

            catch (Exception ex)
            {
                DALLoyalty.Utilidades.Loguear.Error(ex, this.Usuario.ClaveUsuario);
                X.Msg.Alert("Aplicar Membresia", "Ocurrió un Error al Actualizar a Membresia del Cliente").Show();
            }
        }
    }
}
...
{
    public partial class Reporte_Clientes_v2 : PaginaBaseCAPP
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    StoreNivelLealtad.DataSource = DAOReportes.ObtieneCatalogoNivelesLealtad();
        ...
        protected void LlenaGridPanelClientes(int RegistroInicial, string Columna, SortDirection Orden)
        {
            ...
            int? nivelLealtad, estatusUsuario;
            int status = 0;
            ...
            try
            {
                ...
                if (dtClientes == null)
                { 
                    dominioCorreo = txtDominioCorreo.Text == "" ? null : txtDominioCorreo.Text;

                    nivelLealtad = (comboNivelLealtad.Value == null || comboNivelLealtad.Value.ToString() == "-1") ? ((int?)null) : int.Parse(comboNivelLealtad.Value.ToString());

                    estatusUsuario = (comboEstatusUsuario.Value == null || comboEstatusUsuario.Value.ToString() == "-1") ? ((int?)null) : int.Parse(comboEstatusUsuario.Value.ToString());

                    dtClientes = DAOReportes.ObtenerReporteClientesV2(this.Usuario, fechaInicial, fechaFinal, status,
                        dominioCorreo, nivelLealtad, estatusUsuario);
                ...
                else if (dtClientes.Rows.Count > maxRegistros)
                {
                    X.Msg.Confirm("Reporte de Clientes", "La búsqueda arrojó más de " + maxRegistros.ToString() + " registros.</br>" +
                        "El reporte se exportará directamente al archivo Excel.", new MessageBoxButtonsConfig
                        {
                            ...
                            {
                                Handler = "RepClientesMoshi.ClicDePaso()",
                                Text = "Aceptar"
                            },
        ...
        /// </summary>
        [DirectMethod(Namespace = "RepClientesMoshi")]
        ...
        protected IXLWorksheet FormatWsColumns(IXLWorksheet ws, int rowsNum)
        {
            ...
            for (int rowsCounter = 2; rowsCounter <= rowsNum; rowsCounter++)
            {
                ...
                ws.Cell(rowsCounter, 11).SetDataType(XLCellValues.DateTime);

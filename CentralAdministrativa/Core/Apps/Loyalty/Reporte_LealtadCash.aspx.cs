...
{
    public partial class Reporte_LealtadCash : PaginaBaseCAPP
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    HttpContext.Current.Session.Add("DtMovsLealtadCash", null);
                }
            }
        ...
        /// <summary>
        /// Solicita los datos de sucursales a base de datos y los establece en el combo correspondiente
        /// </summary>
        protected void LlenaSucursales()
        {
            try
            {
                ...
                StoreSucursal.DataSource = DAOReportes.ListaSucursalesCash(this.Usuario,
            ...
            catch (Exception ex)
            {
                ...
                X.Msg.Alert("Reporte de Lealtad", ex.Message).Show();
            }
        }

        /// <summary>
        /// Solicita los datos de tipos de movimientos de lealtad a base de datos
        /// y los establece en el combo correspondiente
        /// </summary>
        protected void LLenaTiposMovimientos()
        {
            try
            {
                ...
                StoreTipoMovimiento.DataSource = DAOReportes.ListaTiposDeMovimientosCash(this.Usuario,
                    Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()));
        ...
        protected void LlenaGridPanelMovimientos(int RegistroInicial, string Columna, SortDirection Orden)
        {
            ...
            try
            {
                ...
                dtMovsCash = HttpContext.Current.Session["DtMovsLealtadCash"] as DataTable;

                if (dtMovsCash == null)
                {
                    dtMovsCash = DAOReportes.ObtenerReporteLealtadCash(datInicio.SelectedDate,
                        datFinal.SelectedDate, cmbSucursal.SelectedItem.Value,
                        String.IsNullOrEmpty(cmbTipoMovimiento.SelectedItem.Value) ? -1 : 
                        Convert.ToInt32(cmbTipoMovimiento.SelectedItem.Value),
                        txtMedioAcceso.Text, this.Usuario, 
                        Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()));

                    HttpContext.Current.Session.Add("DtMovsLealtadCash", dtMovsCash);
                }
                ...
                else if (dtMovsCash.Rows.Count > maxRegistros)
                {
                    X.Msg.Confirm("Reporte de Lealtad", "La búsqueda arrojó más de " + maxRegistros.ToString() + " registros.</br>" +
                        "El reporte se exportará directamente al archivo Excel.", new MessageBoxButtonsConfig
                        {
                            ...
                            {
                                Handler = "RepLealtadCash.ClicDePaso()",
                                Text = "Aceptar"
                            },
        ...
        /// </summary>
        [DirectMethod(Namespace = "RepLealtadCash")]
        ...
        public void ExportDataTableToExcel()
        {
            try
            {
                ...
                DataTable _dtMovimientosCash = HttpContext.Current.Session["DtMovsLealtadCash"] as DataTable;

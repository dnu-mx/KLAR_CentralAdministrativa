    ...
    public partial class Reporte_Operaciones : PaginaBaseCAPP
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    StoreEventos.DataSource = DAOCatalogos.ListaEventosSinParametros(this.Usuario, Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()));
        ...
        private void BuscaDatosAndBind()
        {
            try
            {
                ...
                String tipoColectiva, sucursal, tipoMedioAcceso;
                ...
                //tipoMedioAcceso = txtMTipoTransaccion.Text == "" ? null : txtMTipoTransaccion.Text;
                tipoMedioAcceso = cmbEvento.Value == null ? null : cmbEvento.Value.ToString();


                GridPanel1.GetStore().DataSource = LNReportes.ReporteOperaciones(fechaInicio, fechaFin, tipoColectiva, sucursal, tipoMedioAcceso, this.Usuario, Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()));

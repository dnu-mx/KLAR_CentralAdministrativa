...
namespace CentroContacto
{
    public class Global : System.Web.HttpApplication
    {
        ...
        {
            ///LOG HEADER
            LogHeader LH_GlobalLoyalty = new LogHeader();
            var ts = DateTime.Now.ToString("yyyyMMddHHmmss");
            GlobalContext.Properties["ts"] = ts;

            XmlConfigurator.Configure();

            LH_GlobalLoyalty.Application_ID = Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString());
            LH_GlobalLoyalty.Trace_ID = Guid.NewGuid();

            LogPCI log = new LogPCI(LH_GlobalLoyalty);
            log.Info("INICIA Loyalty_Application_Start()");
            ...
            try
            {
                ...
                ValoresInicial.InicializarContexto(LH_GlobalLoyalty);

                log.Info("TERMINA Loyalty_Application_Start()");
            }
            catch (Exception ex)
            {

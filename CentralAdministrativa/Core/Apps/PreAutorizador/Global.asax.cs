...
namespace PreAutorizador
{
    public class Global : System.Web.HttpApplication
    {
        ...
        {
            ///LOG HEADER
            LogHeader LH_GlobalPreautorizador = new LogHeader();
            var ts = DateTime.Now.ToString("yyyyMMddHHmmss");
            GlobalContext.Properties["ts"] = ts;

            XmlConfigurator.Configure();

            LH_GlobalPreautorizador.Application_ID = Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString());
            LH_GlobalPreautorizador.Trace_ID = Guid.NewGuid();

            LogPCI log = new LogPCI(LH_GlobalPreautorizador);
            log.Info("INICIA Preautorizador_Application_Start()");
            ...
            try
            {
                ...
                ValoresInicial.InicializarContexto(LH_GlobalPreautorizador);

                log.Info("TERMINA Preautorizador_Application_Start()");
            }

<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="Reporte_OperacionesApiCacao.aspx.cs" Inherits="OperacionesEvertec.Reporte_OperacionesApiCacao" %>
    ...
    <ext:BorderLayout ID="BorderLayout1" runat="server">
            ...
            <ext:FormPanel ID="FormPanel1" Width="320" Title="Selecciona los Filtros" runat="server" Padding="10"
                ...
                <Content>
                    <ext:Store ID="StoreClientes" runat="server">
                        <Reader>
                            <ext:JsonReader IDProperty="ID_ClienteCacao">
                                <Fields>
                                    <ext:RecordField Name="ID_ClienteCacao" />
                                    <ext:RecordField Name="Clave" />
                                    <ext:RecordField Name="NombreCliente" />
                                </Fields>
                            </ext:JsonReader>
                        </Reader>
                        <SortInfo Field="NombreCliente" Direction="ASC" />
                    </ext:Store>
                    <ext:Store ID="StoreEstatusAut" runat="server">
                        <Reader>
                            <ext:JsonReader IDProperty="ID_EstatusAutorizacion">
                                <Fields>
                                    <ext:RecordField Name="ID_EstatusAutorizacion" />
                                    <ext:RecordField Name="Clave" />
                ...
                <Items>
                    <ext:ComboBox ID="cBoxClienteCacao" runat="server" FieldLabel="SubEmisor" Width="250"
                        StoreID="StoreClientes" DisplayField="NombreCliente" ValueField="ID_ClienteCacao"
                        Mode="Local" AutoSelect="true" ForceSelection="true" TypeAhead="true" MinChars="1"
                        MatchFieldWidth="false" EmptyText="Todos" AllowBlank="false">
                        <Items>
                            <ext:ListItem Text="( Todos )" Value="-1" />
                        </Items>
                    </ext:ComboBox>
                    <ext:DateField ID="dfFIniApiCacao" runat="server" Vtype="daterange" FieldLabel="Fecha Inicial Operación"
                        Format="yyyy/MM/dd" Width="300" EnableKeyEvents="true">
                        <CustomConfig>
                            <ext:ConfigItem Name="endDateField" Value="#{dfFFinApiCacao}" Mode="Value" />
                        </CustomConfig>
                        <Listeners>
                            <KeyUp Fn="onKeyUp" />
                        </Listeners>
                    </ext:DateField>
                    <ext:DateField ID="dfFFinApiCacao" runat="server" Vtype="daterange" FieldLabel="Fecha Final Operación"
                        Width="300" Format="yyyy/MM/dd" EnableKeyEvents="true" >
                        <CustomConfig>
                            <ext:ConfigItem Name="startDateField" Value="#{dfFIniApiCacao}" Mode="Value" />
                        </CustomConfig>
                        <Listeners>
                            <KeyUp Fn="onKeyUp" />
                        </Listeners>
                    </ext:DateField>
                    <ext:DateField ID="dfFIniPresen" runat="server" Vtype="daterange" FieldLabel="Fecha Inicial Presentación"
                        Format="yyyy/MM/dd" Width="300" EnableKeyEvents="true">
                        <CustomConfig>
                            <ext:ConfigItem Name="endDateFieldP" Value="#{dfFFinPresen}" Mode="Value" />
                        </CustomConfig>
                        <Listeners>
                            <KeyUp Fn="onKeyUpPres" />
                        </Listeners>
                    </ext:DateField>
                    <ext:DateField ID="dfFFinPresen" runat="server" Vtype="daterange" FieldLabel="Fecha Final Presentación"
                        Width="300" Format="yyyy/MM/dd" EnableKeyEvents="true">
                        <CustomConfig>
                            <ext:ConfigItem Name="startDateFieldP" Value="#{dfFIniPresen}" Mode="Value" />
                        </CustomConfig>
                        <Listeners>
                            <KeyUp Fn="onKeyUpPres" />
                        </Listeners>
                    </ext:DateField>
                    <ext:DateField ID="dfFIniProc" runat="server" Vtype="daterange" FieldLabel="Fecha Inicial Procesamiento"
                        Format="yyyy/MM/dd" Width="300" EnableKeyEvents="true">
                        <CustomConfig>
                            <ext:ConfigItem Name="endDateFieldC" Value="#{dfFFinProc}" Mode="Value" />
                        </CustomConfig>
                        <Listeners>
                            <KeyUp Fn="onKeyUpComp" />
                        </Listeners>
                    </ext:DateField>
                    <ext:DateField ID="dfFFinProc" runat="server" Vtype="daterange" FieldLabel="Fecha Final Procesamiento"
                        Width="300" Format="yyyy/MM/dd" EnableKeyEvents="true">
                        <CustomConfig>
                            <ext:ConfigItem Name="startDateFieldC" Value="#{dfFIniProc}" Mode="Value" />
                        </CustomConfig>
                        <Listeners>
                            <KeyUp Fn="onKeyUpComp" />
                        </Listeners>
                    </ext:DateField>
                    <ext:TextField ID="txtTarjeta" runat="server" FieldLabel="Tarjeta" EmptyText="Todas" MaskRe="[0-9]"
                        MinLength="16" MaxLength="16" Width="300" />
                    <ext:ComboBox ID="cBoxEstatusOp" FieldLabel="Estatus Operación" StoreID="StoreEstatusAut"
                        EmptyText="Todos" Width="300" runat="server" DisplayField="Descripcion" ListWidth="200"
                        ValueField="ID_EstatusAutorizacion">
                        <Items>
                            ...
                            <ext:Button ID="btnBuscarHide" runat="server" Hidden="true">
                                <Listeners>
                                    <Click Handler="Ext.net.Mask.show({ msg : 'Obteniendo Operaciones...' });
                                        #{GridOpsApiCacao}.getStore().reload({params:{start:0, sort:('','')}});" />
                                </Listeners>
                            </ext:Button>
                            <ext:Button ID="btnBuscar" runat="server" Text="Buscar..." Icon="Magnifier">
                                <DirectEvents>
                                    <Click OnEvent="btnBuscar_Click" Timeout="360000"
                                        ...
                                        else if (!#{dfFIniApiCacao}.getValue() && !#{dfFFinApiCacao}.getValue()
                                        && !#{dfFIniPresen}.getValue() && !#{dfFFinPresen}.getValue()
                                        && !#{dfFIniProc}.getValue() && !#{dfFFinProc}.getValue()) 
                                        { Ext.Msg.alert('Aviso', 'Ingresa al menos una fecha de filtro.');
                                        return false; }
                                        else { resetToolbar(#{PagingOpsApiCacao});
                                        #{GridOpsApiCacao}.getStore().sortInfo = null; }">
                                    </Click>
                                </DirectEvents>
                            </ext:Button>
        ...
        <Center Split="true" Collapsible="false">
            <ext:FormPanel ID="FormPanelOperaciones" runat="server" Layout="FitLayout" Title="Operaciones obtenidas con los filtros seleccionados">
                <Items>
                    <ext:GridPanel ID="GridOpsApiCacao" runat="server" StripeRows="true" Header="false" Border="false">
                        <LoadMask ShowMask="false" />
                        <Store>
                            <%--OnSubmitData="StoreOpsApiCacao_Submit"--%>
                            <ext:Store ID="StoreOpsApiCacao" runat="server" RemoteSort="true" AutoLoad="false"
                                OnRefreshData="StoreOpsApiCacao_RefreshData">
                                <AutoLoadParams>
                                ...
                                <Reader>
                                    <ext:JsonReader IDProperty="ID_Operacion">
                                        <Fields>
                                            ...
                                            <ext:RecordField Name="MonedaOrigen" />
                        ...
                        <BottomBar>
                            <ext:PagingToolbar ID="PagingOpsApiCacao" runat="server" StoreID="StoreOpsApiCacao" DisplayInfo="true"

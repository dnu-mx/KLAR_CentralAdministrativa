...
namespace OperacionesEvertec
{
    public class Global : System.Web.HttpApplication
    {
        ...
        {
            ///LOG HEADER
            LogHeader LH_GlobalOpsEvtc = new LogHeader();
            var ts = DateTime.Now.ToString("yyyyMMddHHmmss");
            GlobalContext.Properties["ts"] = ts;

            XmlConfigurator.Configure();

            LH_GlobalOpsEvtc.Application_ID = Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString());
            LH_GlobalOpsEvtc.Trace_ID = Guid.NewGuid();

            LogPCI log = new LogPCI(LH_GlobalOpsEvtc);
            log.Info("INICIA OperacionesEvertec_Application_Start()");
            ...
            try
            {
                ...
                ValoresInicial.InicializarContexto(LH_GlobalOpsEvtc);

                log.Info("TERMINA OperacionesEvertec_Application_Start()");
            }

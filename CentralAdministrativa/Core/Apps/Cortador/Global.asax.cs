...
namespace Cortador
{
    public class Global : System.Web.HttpApplication
    {
        ...
        {
            ///LOG HEADER
            LogHeader LH_GlobalEventos = new LogHeader();
            var ts = DateTime.Now.ToString("yyyyMMddHHmmss");
            GlobalContext.Properties["ts"] = ts;

            XmlConfigurator.Configure();

            LH_GlobalEventos.Application_ID = Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString());
            LH_GlobalEventos.Trace_ID = Guid.NewGuid();

            LogPCI log = new LogPCI(LH_GlobalEventos);
            log.Info("INICIA Eventos_Application_Start()");
            ...
            try
            {
                ...
                ValoresInicial.InicializarContexto(LH_GlobalEventos);

                log.Info("TERMINA Eventos_Application_Start()");
            }
            catch (Exception ex)
            {

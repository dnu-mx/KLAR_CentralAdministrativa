    ...
    public partial class EditOperadores : PaginaBaseCAPP
    {
        ...
        private void BindDataColectiva()
        {
            ...
            try
            {
                ...
                //if (this.txtBuscarNombre.Text.Trim().Length + this.txtBuscarPaterno.Text.Trim().Length + this.txtBuscarUsuario.Text.Trim().Length < 5)
                //{
                //    X.Msg.Alert("Buscar Colectiva", "Por favor ingresa los datos a buscar, mínimo 5 caracteres ").Show();
                //    return;
                //}

                GridPanel1.GetStore().DataSource = DAOCatalogos.ListaOperadoresPorTipoColectiva(this.txtBuscarNombre.Text, this.txtBuscarPaterno.Text, this.txtBuscarUsuario.Text, this.Usuario, Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString()), DALCentralAplicaciones.Utilidades.Configuracion.Get(Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()), "ClaveTipoColectivaOperador").Valor);

(function (zmodule) {
            ...
            $scope.getElementsInidencias = function () {
                ...
                serviceQueries.getIncidenciasApi($scope.params,
                    function (result) {
                        ...
                        $scope.getElementsTarjetas();
                    });
            }



            $scope.getElementsTarjetas = function () {

                serviceLoading.start();
                serviceQueries.getTarjetasApi($scope.params,
                    function (result) {

                        //console.log(result.data);
                        $scope.chart6.Data = result.Data;
                        $scope.chart6.Series = result.Series;
                        $scope.chart6.Labels = result.Labels;
                        //$scope.chart1.DataAmount = result.DataAmount;
    ...
    [
        "Restangular", "service.messages", function (Restangular, serviceMessages) {
            ...
            service.getTarjetasApi = function (dataQuery, fnRespuesta) {
                var val = {};
                //Restangular.all("Queries").all("GetGraficaTransacciones").get(dataQuery).then(fnRespuesta);

                return all.customGET("GetGraficaTarjetas", dataQuery).then(fnRespuesta);

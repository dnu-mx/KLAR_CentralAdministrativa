...
namespace TpvWeb
{
    public partial class AdministrarColectivasToken : PaginaBaseCAPP
    {
        /// <summary>
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    DataSet dsTiposColectiva = DAOColectiva.ListaTiposColectivaCacao
        ...
        protected void LimpiaSeleccionPrevia()
        {
            ...
            cBoxClasificacion.Reset();
            cBoxParametros.Reset();
            cBoxParametros.Disabled = true;
            btnAddParametros.Disabled = true;
            StoreParametros.RemoveAll();
        ...
        protected void selectRowResultados_Event(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                LlenaFormPanelTokens(IdColectiva);
        ...
        /// <param name="sender">Objeto que envía el control</param>
        /// <param name="e">Argumentos del evento que se ejecutó</param>
        protected void btnNuevoTokenAcceso_Click(object sender, EventArgs e)
        {
            try
            {
                NewToken token = new NewToken();

                if (!TokenPassword.Text.Equals(TokenPasswordValidate.Text))
                {
                    X.Msg.Alert("Añadir Token", "La confirmación de contraseña y contraseña deben coincidir").Show();
                    return;
                }


                token.colectivaId = Convert.ToInt64(this.hdnIdColectiva.Text);
                token.description = TokenDescripcion.Text;
                token.email = TokenEmail.Text;
                token.name = TokenName.Text;
                token.password = TokenPassword.Text;

                WsResponse resp = LNAdministrarColectivas.CreaNuevoTokenAColectiva(token, Usuario);
                if (resp.success)
                {
                    X.Msg.Notify("","Token Añadido"+ String.Join("<br/>", resp.Messages)).Show();
                    LlenaFormPanelTokens(int.Parse(this.hdnIdColectiva.Text));
                }
                else
                {
                    X.Msg.Alert("Añadir Token", String.Join("<br/>", resp.Messages)).Show();
                }
            ...
            {
                X.Msg.Alert("Añadir Token", err.Mensaje()).Show();
            }

            catch (Exception ex)
            {
                Loguear.Error(ex, this.Usuario.ClaveUsuario);
                X.Msg.Alert("Añadir Token", ex.Message).Show();
            }
        }

        private void LlenaFormPanelTokens(int IdColectiva)
        {
            try
            {
                FormPanelToken.Reset();

                TokenStore.DataSource = DAOAdministrarColectivas.ConsultaTokensColectiva(IdColectiva,
                    this.Usuario, Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString()));
                TokenStore.DataBind();

                TokenAnadir.Disabled = false;
            }
            catch (CAppException caEx)
            {
                Loguear.Error(caEx, this.Usuario.ClaveUsuario);
        ...
        /// <param name="sender">Objeto que envía el control</param>
        /// <param name="e">Argumentos ddirectos el evento que se ejecutó</param>
        protected void EjecutarComandoToken(object sender, DirectEventArgs e)
        {
            try
            {
                String comando = (String)e.ExtraParams["Comando"];
                HttpContext.Current.Session.Add("TokenId", Convert.ToInt32(e.ExtraParams["TokenId"]));
                HttpContext.Current.Session.Add("ColectivaId", Convert.ToInt32(e.ExtraParams["TokenId"]));
                char[] charsToTrim = { '*', '"', ' ' };

                switch (comando)
                {
                    case "Get":
                        String emailDelToken = e.ExtraParams["Email"].ToString().Replace("\"",string.Empty);

                        txtCorreos.Text = emailDelToken;
                    ...
                    case "Delete":
                        DeleteToken(Convert.ToInt32(e.ExtraParams["TokenId"].ToString()));
        ...
        private void DeleteToken(int tokenId)
        {
            try
            {
                LNAdministrarColectivas.DeleteToken(tokenId, this.Usuario);
                LlenaFormPanelTokens(int.Parse(this.hdnIdColectiva.Text));
                X.Msg.Notify("", String.Join("<br/>", "Token Eliminado" + "<br />  <br /> <b> E X I T O S A M E N T E </b> <br />  <br /> ")).Show();
            }
            catch(Exception ex)
            {
                Loguear.Error(ex, this.Usuario.ClaveUsuario);
        ...
        protected void EnviarCorreo(object sender, DirectEventArgs e)
        {
            try
            {
                Int32 TokenId = (Int32)HttpContext.Current.Session["TokenId"]; ;

                if (String.IsNullOrEmpty(txtCorreos.Text))
                {
                    X.Msg.Alert("Acción", "Debe ingresar almenos un destinatario").Show();
                    return;
                }

                LNAdministrarColectivas.SendTokenByEmail(txtCorreos.Text.ToString().Replace("\"",string.Empty), TokenId,
                    this.Usuario, Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString()));


                X.Js.AddScript("#{frmSendMail}.setVisible(false);");

                X.Msg.Notify("", String.Join("<br/>", "Token Enviado" + "<br />  <br /> <b> E X I T O S A M E N T E </b> <br />  <br /> ")).Show();
            }
            catch(Exception ex)
            {
                Loguear.Error(ex, this.Usuario.ClaveUsuario);
        ...
        protected void LlenaParametrosColectiva()
        {
            try
            {
                ...
                StoreParametros.DataSource = DAOAdministrarColectivas.ListaParametrosSinAsignar(
                    int.Parse(cBoxClasificacion.SelectedItem.Value), IdColectiva, this.Usuario,
                    Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString()));
                StoreParametros.DataBind();

                StoreValoresParametros.DataSource = DAOAdministrarColectivas.ListaParametrosAsignados(
                    int.Parse(cBoxClasificacion.SelectedItem.Value), IdColectiva, this.Usuario,
                    Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString()));
            ...
            {
                Loguear.Error(ex, this.Usuario.ClaveUsuario);
                X.Msg.Alert("Parámetros", ex.Message).Show();
            }
        }
        ...
        protected void btnGuardarValorParametro_Click(object sender, EventArgs e)
        {
            ...
            {
                Loguear.Error(ex, this.Usuario.ClaveUsuario);
                X.Msg.Alert("Actualización de Parámetros", "Error al Actualizar el Valor del Parámetro de Contrato").Show();
            }
        }
        ...
        /// </summary>
        protected void ActualizaValorParametro()
        {
            Int64 IdColectiva = Convert.ToInt64(this.hdnIdColectiva.Text);

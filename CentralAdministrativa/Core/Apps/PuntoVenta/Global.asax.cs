...
namespace TpvWeb
{
    public class Global : System.Web.HttpApplication
    {
        ...
        {
            ///LOG HEADER
            LogHeader LH_GlobalPtoVta = new LogHeader();
            var ts = DateTime.Now.ToString("yyyyMMddHHmmss");
            GlobalContext.Properties["ts"] = ts;

            XmlConfigurator.Configure();

            LH_GlobalPtoVta.Application_ID = Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString());
            LH_GlobalPtoVta.Trace_ID = Guid.NewGuid();

            LogPCI log = new LogPCI(LH_GlobalPtoVta);
            log.Info("INICIA PuntoVenta_Application_Start()");
            ...
            try
            {
                ...
                ValoresInicial.InicializarContexto(LH_GlobalPtoVta);

                log.Info("TERMINA PuntoVenta_Application_Start()");
            }

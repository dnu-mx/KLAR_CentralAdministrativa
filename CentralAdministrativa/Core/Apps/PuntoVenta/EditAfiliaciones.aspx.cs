...
namespace TpvWeb
{
    public partial class EditAfiliaciones : PaginaBaseCAPP
    {
        ...
        protected void PreparaGridColectiva()
        {
            ...
            Bloquear.Icon = Icon.RecordGreen;
            Bloquear.CommandName = "Bloquear";
            Bloquear.ToolTip.Text = "Inactivar Afiliación";
            ...
            play.ToolTip.Text = "Activar Afiliación";
        ...
        protected void GridEmpleados_DblClik(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                this.hdnIdColectiva.Value = ID_colectiva;
                DespliegaDatosColectiva(ID_colectiva);
            }
            catch (Exception ex)
            {
                X.Msg.Alert("Afiliación", "Ocurrio un error al seleccionar la Afiliación. " + ex.Message).Show();
            }
        ...
        protected void DespliegaDatosColectiva(Int64 ID_Colectiva)
        {
            ...
            FormPanel1.Title = "Editando Afiliación Clave - " + laColectivaEditar.ClaveColectiva;
        ...
        protected Colectiva AsignarNuevosDatos(Colectiva laColectiva)
        {
            ...
            laColectiva.Password = txtClaveSupervisor.Text;
        ...
        /// <summary>
        /// Solicita y establece la información de claves extra de colectiva.
        /// </summary>
        protected void LlenaGridInfoAd()
        {
            try
            {
                this.StoreClavesExtra.RemoveAll();

                this.StoreClavesExtra.DataSource = DAOColectiva.ObtieneClavesExtraColectiva(
                    Convert.ToInt32(this.hdnIdColectiva.Value), this.Usuario);
                this.StoreClavesExtra.DataBind();
            }
            catch (CAppException caEx)
            {
                X.Msg.Alert("Claves Adicionales", caEx.Mensaje()).Show();
            }
            catch (Exception ex)
            {
                X.Msg.Alert("Claves Adicionales", ex.Message).Show();
            }
        }

        /// <summary>
        /// Controla el evento Click al botón Aceptar de la ventana de nueva clave
        /// </summary>
        /// <param name="sender">Objeto que envía el control</param>
        /// <param name="e">Argumentos del evento directo que se ejecutó</param>
        protected void btnAddNuevaClave_Click(object sender, DirectEventArgs e)
        {
            try
            {
                LNColectiva.CreaClaveColectivaExtra(Convert.ToInt32(this.hdnIdColectiva.Value),
                    this.txtNuevaClave.Text, this.dfNuevaFechaAs.SelectedDate, this.Usuario);

                this.WdwNuevaClave.Hide();

                X.Msg.Notify("Nueva Clave", "Clave Adicional creada <br />  <br /> <b> E X I T O S A M E N T E</b> <br />  <br /> ").Show();

                LlenaGridInfoAd();
            }
            catch (Exception ex)
            {
                X.Msg.Alert("Nueva Clave", ex.Message).Show();
            }
        }
        ...
        /// 
        /// </summary>
        /// <param name="IdColectivasCE"></param>
        /// <param name="Clave"></param>
        /// <param name="fechaAsig"></param>
        [DirectMethod(Namespace = "EditaAfiliaciones")]
        public void ActualizarClaveExtra(int IdColectivaCE, string Clave, DateTime fechaAsig)
        {
            try
            {
                LNColectiva.ModificaClaveColectivaExtra(
                    IdColectivaCE, Clave, fechaAsig, this.Usuario);

                X.Msg.Notify("Clave Adicional", "Clave Adicional modificada <br />  <br /> <b> E X I T O S A M E N T E</b> <br />  <br /> ").Show();
            }
            catch (Exception ex)
            {
                X.Msg.Alert("Clave Adicional", ex.Message).Show();
            }
        }
    }
}
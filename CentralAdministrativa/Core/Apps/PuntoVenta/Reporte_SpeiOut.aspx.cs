...
namespace TpvWeb
{
    public partial class Reporte_SpeiOut: PaginaBaseCAPP
    {
        #region Variables privadas

        //LOG HEADER Parabilia Reporte SpeiOut
        private LogHeader LH_ParabRepSpeiOut = new LogHeader();

        #endregion

        /// <summary>
        /// Realiza y controla la carga de la página Reporte de SpeiOut
        /// </summary>
        ...
        {
            ///LOG HEADER
            LH_ParabRepSpeiOut.IP_Address = Request.ServerVariables["REMOTE_ADDR"].ToString();
            LH_ParabRepSpeiOut.Application_ID = Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString());
            LH_ParabRepSpeiOut.User = this.Usuario.ClaveUsuario;
            LH_ParabRepSpeiOut.Trace_ID = Guid.NewGuid();

            LogPCI log = new LogPCI(LH_ParabRepSpeiOut);
            string errRedirect = string.Empty;

            try
            {
                log.Info("INICIA ReporteSpeiOut Page_Load()");
                ...
                log.Info("TERMINA ReporteSpeiOut Page_Load()");
            }
        ...
        {
            LogPCI pCI = new LogPCI(LH_ParabRepSpeiOut);
        ...
        public void ExportDataTableToExcel()
        {
            try
            {
                string reportName = "ReporteSpeiOut";

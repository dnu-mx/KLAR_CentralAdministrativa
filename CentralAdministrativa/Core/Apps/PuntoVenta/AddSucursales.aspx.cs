...
namespace TpvWeb
{
    public partial class AddSucursales : PaginaBaseCAPP
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    try
                    {
                        cmbTipoUsuario.SetValueAndFireSelect(DALCentralAplicaciones.Utilidades.Configuracion.Get(Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()), "ClaveTipoColectivaSucursal").Valor);
                        cmbTipoUsuario.SetValue(DALCentralAplicaciones.Utilidades.Configuracion.Get(Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()), "ClaveTipoColectivaSucursal").Valor);
        ...
        protected void LlenaColectivasPosiblePadre(Int64 ID_Padre, String ClaveTipoColectivaHijo)
        {
            try
            {
                DataSet dsCadenas = DAOCatalogos.ListaColectivaPosiblePadre(ID_Padre, ClaveTipoColectivaHijo, this.Usuario, Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString()));
                ...
                SCPadre.DataSource = ListaCadenas;
                SCPadre.DataBind();
            }
            catch (Exception )
            {
            ...
            try
            {
                ...
                switch (EjecutarComando)
                {
                    ...
                    case "Bloquear":
                        ...
                        {
                            throw new Exception("No se Desactivo la Sucursal");
                        }
                        break;
                    case "Activar":

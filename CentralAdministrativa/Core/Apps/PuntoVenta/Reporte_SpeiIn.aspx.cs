...
namespace TpvWeb
{
    public partial class Reporte_SpeiIn : PaginaBaseCAPP
    {
        #region Variables privadas

        //LOG HEADER Parabilia Reporte SpeiIN
        private LogHeader LH_ParabRepSpeiIn = new LogHeader();

        #endregion

        /// <summary>
        /// Realiza y controla la carga de la página Reporte de SpeiIN
        /// </summary>
        ...
        {
            ///LOG HEADER
            LH_ParabRepSpeiIn.IP_Address = Request.ServerVariables["REMOTE_ADDR"].ToString();
            LH_ParabRepSpeiIn.Application_ID = Guid.Parse(ConfigurationManager.AppSettings["IdApplication"].ToString());
            LH_ParabRepSpeiIn.User = this.Usuario.ClaveUsuario;
            LH_ParabRepSpeiIn.Trace_ID = Guid.NewGuid();

            LogPCI log = new LogPCI(LH_ParabRepSpeiIn);
            string errRedirect = string.Empty;

            try
            {
                log.Info("INICIA ReporteSpeiIn Page_Load()");
                ...
                log.Info("TERMINA ReporteSpeiIn Page_Load()");
            }
        ...
        {
            LogPCI pCI = new LogPCI(LH_ParabRepSpeiIn);
            ...
            try
            {
                ...
                if (dtFechasActTarj == null)
                {
                    ...
                    dtFechasActTarj = DAOReportes.ReporteTraspasosSpei(
                        ...
                        Convert.ToDateTime(dfFechaFin.SelectedDate), true, this.Usuario,
        ...
        public void ExportDataTableToExcel()
        {
            try
            {
                string reportName = "ReporteSpeiIn";

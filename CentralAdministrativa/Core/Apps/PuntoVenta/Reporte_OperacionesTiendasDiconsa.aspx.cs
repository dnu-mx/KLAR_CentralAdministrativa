...
namespace TpvWeb
{
    public partial class Reporte_OperacionesTiendasDiconsa : PaginaBaseCAPP
    {
        ...
        private void BuscaDatosAndBind()
        {
            try
            {
                ...
                String  Estatus, Telefono, Benefiario;
                ...
                Benefiario = cmbBeneficiario.Value == null ? "" : cmbBeneficiario.Value.ToString();
                Telefono = txtTelefono.Text.Trim().Length == 0 ? "" : txtTelefono.Text.Trim();

                GridPanel1.GetStore().DataSource = LNReportes.ListarOperacionesTiendaDiconsa(fechaInicial, FechaFinal, 
                    Sucursal, UnidadOperativa, Almacen, Benefiario, Estatus, Telefono, IdTienda, 
        ...
        protected void LlenaBeneficiario()
        {
            try
            {
                ...
                StoreBeneficiario.DataSource = DAOCatalogos.ObtieneColectivasParaFiltrosReporteVentasTienda
                    (DALCentralAplicaciones.Utilidades.Configuracion.Get(Guid.Parse(
                    ConfigurationManager.AppSettings["IDApplication"].ToString()), 
                    "ClaveTipoColectivaBeneficiario").Valor, -1, this.Usuario,
                    Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()));

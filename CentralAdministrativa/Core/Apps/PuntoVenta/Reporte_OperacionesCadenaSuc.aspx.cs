...
namespace TpvWeb
{
    public partial class Reporte_OperacionesCadenaSuc : PaginaBaseCAPP
    {
        ...
        private void BuscaDatosAndBind()
        {
            try
            {
                ...
                GridPanel1.GetStore().DataSource = LNOperaciones.ListarOperacionesCadenaFiltroExtraSuc(fechaInicial, FechaFinal, Sucursal, Afiliacion, Terminal, Operador, Benefiario, Estatus, Telefono, ID_CadenaComercial, this.Usuario, Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()));
        ...
        protected void LlenaSucursales(object sender, EventArgs e)
        {
            try
            {
                ...
                //cmbTerminal.Reset();
                //cmbOperador.Reset();
                StoreSucursal.DataSource = DAOCatalogos.ObtieneColectivasParaFiltrosReportesFiltroSuc(this.Usuario, Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()), DALCentralAplicaciones.Utilidades.Configuracion.Get(Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()), "ClaveTipoColectivaSucursal").Valor, "0", Int64.Parse(cmbCadenaComercial.Value.ToString()));

...
namespace Facturas
{
    public partial class Reporte_EventosEjecutados : PaginaBaseCAPP
    {
        ...
        private void BuscaDatosAndBind()
        {
            try
            {
                ...
                bool isLimiteInfValido = txtImporteInferior.Text.Trim().Length != 0 ? float.TryParse(txtImporteInferior.Text.Trim(), out MontoInferior) : false;
                bool isLimiteSupValido = txtImporteSuperior.Text.Trim().Length != 0 ? float.TryParse(txtImporteSuperior.Text.Trim(), out MontoSuperior) : false;

                if (isLimiteInfValido | isLimiteSupValido)
                {
                    throw new Exception("Monto Invalido");
                }

                GridPanel1.GetStore().DataSource = LNPoliza.PolizasParaFactura(fechaInicial, FechaFinal, MontoInferior,
                    MontoSuperior, ID_CadenaComercial, this.Usuario, Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()));

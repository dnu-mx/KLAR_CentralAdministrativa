...
namespace Facturas
{
    public partial class ConsultarMisFacturas : PaginaBaseCAPP
    {
            ...
            try
            {
                ...
                switch (EjecutarComando)
                {
                    ...
                    case "Timbrar":
                        ...
                        int resp = LNTimbrados.TimbrarFactura(LaFactura, XmlFactura.ToString(), this.Usuario);
                        ...
                        if (resp == 0)
                        {
                            ...
                            {
                                LNFactura.CambiaEstatusFactura(ID_Factura, 5);//Cambia el estatus a Timbrado
        ...
        private void BuscaDatosAndBind()
        {
            try
            {
                ...
                DataSet dsFacturas = DAOFactura.ConsultaFacturas(datInicio.SelectedDate, datFinal.SelectedDate, ID_Emisor, ID_Receptor,
                    ID_Estatus, this.Usuario, Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()));
        ...
        protected void EnviarCorreo(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                if (LaFacturaToSend.TipoComprobante.ToUpper().Equals("INGRESO"))
                {
        ...
        //        String laFacturaPDF = Configuracion.Get(Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()), "urlLasFacturas").Valor + "Factura_" + LaFacturaToSend.Serie + LaFacturaToSend.Folio + ".pdf";
        //        String LaFacturaXML = Configuracion.Get(Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()), "urlLasFacturas").Valor + "Factura_" + LaFacturaToSend.Serie + LaFacturaToSend.Folio + ".xml";
        //        String elHtml = Configuracion.Get(Guid.Parse(ConfigurationManager.AppSettings["IDApplication"].ToString()), "Mailhtml").Valor + "MailParaEnvioFacturas.html";
